...
class MyNavigator(BaseNavigator):
    ...
    def compute_trajectory_tracking_control(self,
        ...
        th = np.arctan2(yd_d, xd_d)

        ########## Code starts here ##########
        u_1 = xdd_d + self.kpx * ( x_d - x ) + self.kdx * ( xd_d - self.V_prev * np.cos(th))
        u_2 = ydd_d + self.kpy * ( y_d - y ) + self.kdy * ( yd_d - self.V_prev * np.sin(th))

        vdot = u_1 * np.cos(th) + u_2 * np.sin(th)
        if (self.V_prev < self.v_prev_thresh):
            self.V_prev = self.v_prev_thresh
        om = -u_1 * np.sin(th) / self.V_prev  + u_2 * np.cos(th) / self.V_prev
    ...
    def compute_trajectory_plan(self,
        ...
        astar = AStar((-horizon, -horizon), (horizon,  horizon), x_init,  x_goal, occupancy)
        if not astar.solve():
            return None
        path = astar.path
        return self.compute_smooth_plan(path)

    def compute_smooth_plan(path, v_desired=0.15, spline_alpha=0.05) -> TrajectoryPlan:
        path = np.array(path)
        # time is taken to be (distance between previous points)/v_desired
...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        ########## Code starts here ##########
        for x_i in range(3):
            dx = (x_i - 1) * self.resolution
            for y_i in range(3):
                if (x_i == 1 and y_i == 1):
                    continue
                pos = x
                dy = (y_i - 1) * self.resolution
        ...
        while len(self.open_set):
            ...
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh in self.closed_set:
                    continue
                tent_cost = self.cost_to_arrive[x_current] + self.distance(x_current, x_neigh)
                if x_neigh not in self.open_set:
                    self.open_set.add(x_neigh)
                elif tent_cost > self.cost_to_arrive[x_neigh]:
                    continue
                self.came_from[x_neigh] = x_current
                self.cost_to_arrive[x_neigh] = tent_cost
                self.est_cost_through[x_neigh] = tent_cost + self.distance(x_neigh, self.x_goal)
        return False
        ########## Code ends here ##########
